...
COLLECTOR = ROOT / "install-helpers/collect-release-outputs.py"
...
def run(root: Path, plan: dict, name: str, ok: bool) -> subprocess.CompletedProcess[str]:
...
def main() -> None:
    with tempfile.TemporaryDirectory() as raw:
        ...
        for role, (media, payload) in ROLES.items():
            artifact = root / role; write(artifact, payload)
        ...
        for name, item in mutations.items(): run(root, item, name, False)
        ...
        run(root, rejected, "verifier-rejection", False)
        run(root, plan, "existing", True)
