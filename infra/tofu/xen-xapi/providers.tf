# The farm spans FIVE standalone XCP-ng pools; the xenserver provider is
# single-pool, so one aliased provider per dom0 XAPI endpoint. Same root password
# (TF_VAR_xapi_password from /root/.mcnf-xapi-cred, 0600, off-repo) on all five.
provider "xenserver" {
...
provider "xenserver" {
  alias    = "x196" # XEN-196 (4c / 24 GiB, added 2026-08-08)
  host     = "https://172.20.145.196"
  username = var.xapi_username
  password = var.xapi_password
}
