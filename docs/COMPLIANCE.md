...
| lizardfs-admin (Mesh Storage query) | Unprovisioned precondition | Not in F44 repos; the Mesh Storage panel can't query the master without it. **(RETIRED — SUBSTRATE-6: there is no LizardFS master to query; the Mesh Storage panel now reads etcd + Syncthing folder health. Historical row.)** | High | ~~FINISH (AUDIT-MESH-12)~~ → **RETIRED (SUBSTRATE-6)** |
| First-boot fetch model (ntfy/starship/future LizardFS) | Fit-for-purpose caveat | The fetch birthrights need **internet at first boot**; an offline/air-gapped install won't provision them, and there's no bundled fallback. **(The "future LizardFS" fetch is RETIRED — SUBSTRATE-6; the substrate deps are now etcd + syncthing. The ntfy/starship caveat still stands.)** | Med | **FINISH** (BIRTHRIGHT-2: decide bundle-vs-fetch for air-gapped) |
...
**Counts:** Unprovisioned-precondition FINISH = 4 (the LizardFS + lizardfs-admin items are **RETIRED by SUBSTRATE-6** — see the rows above, no longer open; 2 fixed this session: ntfy, starship) · Fit-for-purpose caveat = 1 · OK = 3.

**Bottom line — fit for purpose?** Partially (as of this sweep). The in-repo birthrights are correct, and this session closed the ntfy + starship gaps. The "core shared-state substrate is not a birthright" finding was about **LizardFS** — **now moot: LizardFS is REMOVED (SUBSTRATE-6).** The substrate is etcd (coordination, provisioned at enroll) + Syncthing (files), so the LizardFS-provisioning gap no longer exists. The remaining standing item is the air-gapped-fetch caveat (ntfy/starship).
