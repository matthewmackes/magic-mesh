...
- [>] **PLANES-24: mirrors** — magic-mesh GitHub-RPM mirror dir on LizardFS (W61, pulls the GitHub Releases/Pages RPMs, not COPR); **every node serves itself via `file://` baseurl** + upstream fallback (W62); sync = scheduled one-puller job (W63). **CONFIG MODEL + CATALOG LANDED 2026-06-10:** `mackesd/src/mirrors.rs` (`Mirror`: name/upstream/enabled, the LizardFS local dir + computed `file://` baseurl + a `.last-sync` freshness marker, TOML + a core pack carrying the `magic-mesh` GitHub-Pages mirror); `mackesd mirrors [--json]` verb; the **Provisioning ▸ Mirrors** panel (`panels/mirrors.rs`) renders each mirror's upstream, the `file://` serve URL (W62), enabled state, and sync freshness. /preview-verified with the real magic-mesh mirror. **SYNC PULLER LANDED 2026-06-11:** `mirrors::sync_mirror` (W63) — a `MirrorSyncRunner` trait (mocked in tests like `NebulaCertBackend`) + `SubprocessSync` shelling `dnf reposync --repofrompath` → `createrepo_c` into the mirror's LizardFS dir, then stamping `.last-sync` LAST (a failed pull never falsely reports synced); wired as `mackesd mirrors --sync <name> | --sync-all` (4 sync unit tests). **DNF .REPO REWRITE LANDED 2026-06-11 (W62):** `mirrors::render_dnf_repo` emits the `.repo` with the local `file://` baseurl FIRST + the upstream as a fallback line (dnf tries baseurls in order → self-serve, GitHub fallback); `write_dnf_repo` lands it as `mackes-mirror-<name>.repo`, wired as `mackesd mirrors --write-repo [--repo-dir DIR]` (3 unit tests, smoke-verified). REMAINING: only the *scheduled* one-puller job (a `mackesd` worker on a timer + single-puller coordination so just one node pulls; the sync + serve mechanics now exist, this is the cron-trigger + leader-gate around them).
