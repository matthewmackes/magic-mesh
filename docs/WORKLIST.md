...
- [✓] **PKG-8: signed COPR** — done (metadata) — packaging/repo/magic-mesh.repo (gpgcheck on, COPR project pubkey); the magic-mesh-release RPM ships it. The actual COPR build + key publish are /release-gated — COPR built-in per-project GPG; ship the pubkey + a `magic-mesh-release.rpm` (Q79).
- [✓] **PKG-9: Magic-on-Cosmic ISO** — done (kickstart) — packaging/kickstart/magic-on-cosmic.ks: Fedora-Cosmic spin + nebula + ansible-core + magic-mesh from the COPR, %post bash-validated; the livemedia-creator build is /release-gated — a Fedora-Cosmic kickstart built with livemedia-creator (Q80).
