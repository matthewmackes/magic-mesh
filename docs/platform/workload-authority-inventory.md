...
| `install-helpers/provision-mesh-vm.sh` direct `virt-install` helper | typed Workload create/start operations | the unreferenced helper copied arbitrary images into libvirt storage and created domains outside admission, journaling, and the sole actuator; it was deleted and the authority lint rejects restoration |
