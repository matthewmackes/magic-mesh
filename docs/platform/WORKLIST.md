...
- **Chat production dependency hard-cut (2026-08-15):** legacy Chat shell code is now test-only; production date handling uses the neutral calendar module and `mde-chat` is no longer a normal shell dependency. BigBoy `cargo check -p mde-shell-egui --tests` passed: `evidence/WL-FUNC-011-2026-08-15-chat-production-dependency-r1.md`.
- Remaining work:
...
- **Cast adapter, discovery, command dispatch, ownership, and typed handoff (2026-08-15):** `mde-musicd` now owns a bounded numeric-address Cast target, bounded DIAL identity parser, CASTV2 TLS/protobuf seam, blocking default-media-receiver load/play/pause/seek dispatch, generation-bound atomic ownership records, and a typed Cast handoff request using `rust_cast`; durable peer handoff records carry backward-compatible target kind/identity fields defaulting to `mesh_seat`, and the production completion pump now routes admitted `cast_renderer` targets through Cast load before publishing ownership. Focused Cast tests pass 12/12 and handoff compatibility tests pass 16/16. The reachable Xiaomi MIBOX4 proof is recorded in `evidence/WL-FUNC-021-2026-08-15-cast-target-availability-r1.md`; signed-release/provider continuity proof remains post-release.
