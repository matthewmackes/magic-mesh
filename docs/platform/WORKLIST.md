...
- **Cast adapter, discovery, command dispatch, ownership, and typed handoff (2026-08-15):** `mde-musicd` now owns a bounded numeric-address Cast target, bounded DIAL identity parser, CASTV2 TLS/protobuf seam, blocking default-media-receiver load/play/pause/seek dispatch, generation-bound atomic ownership records, and a typed Cast handoff request using `rust_cast`; focused farm tests pass 11/11 including live connection and media load/pause. The reachable Xiaomi MIBOX4 proof is recorded in `evidence/WL-FUNC-021-2026-08-15-cast-target-availability-r1.md`; integration into the mesh handoff commit remains.
