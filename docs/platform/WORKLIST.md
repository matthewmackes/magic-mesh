...
- **Cast adapter, discovery, command dispatch, and ownership (2026-08-15):** `mde-musicd` now owns a bounded numeric-address Cast target, bounded DIAL identity parser, CASTV2 TLS/protobuf seam, blocking default-media-receiver load/play/pause/seek dispatch, and generation-bound atomic ownership records using `rust_cast`; focused farm tests pass 10/10 including live connection and media load/pause. The reachable Xiaomi MIBOX4 proof is recorded in `evidence/WL-FUNC-021-2026-08-15-cast-target-availability-r1.md`; integration into the mesh handoff commit remains.
...
- **Combined inventory regression gate (2026-08-14):** all 31 device-inventory
