...
- Combined device-inventory gate: `cargo test -p mackesd --features async-services workers::device_inventory::tests -- --nocapture` — PASS, 31/31 provider/inventory tests; 4,972 unrelated tests filtered.
...
- Fixture-root enumeration now excludes host CUPS/systemd observations, keeping
  empty-host tests deterministic without weakening production `/sys` + `/proc`
  provider behavior.
