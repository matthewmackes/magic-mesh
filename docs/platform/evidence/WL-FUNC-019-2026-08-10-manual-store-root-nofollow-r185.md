...
- Scope: Remote Sessions manual-source persistence refuses a symlinked store directory before creating or replacing its JSON store.
...
- Result: `1 passed; 0 failed; 4718 filtered out` on `.50`; the hostile target directory remained without a created manual-store file.
