...
- The remaining three provider checks also passed in isolated `.90` slots: `typed_bookmark_uses_the_selected_admitted_provider`, `typed_playlist_mutation_uses_the_selected_admitted_provider`, and `typed_playlist_actions_use_the_admitted_provider` (1/1 each). All five provider failures are therefore isolated-only fixture interaction.
