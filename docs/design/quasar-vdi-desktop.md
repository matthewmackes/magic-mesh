...
```
  ...
  │    local VMs: cloud-hypervisor (virtio-gpu; dual-homed mesh peer + LAN NIC)│
...
- **R2 — cloud-hypervisor + Windows + virtio-gpu + passthrough.** cloud-hypervisor's Windows/GPU story is less trodden than QEMU's. *Mitigate:* pre-tooled golden image (virtio drivers); RDP/VNC fallback if the virtio-gpu fast path lags; passthrough is operator-opt per host.
...
5. **Local cloud-hypervisor** — `mde-kvm` + virtio-gpu fast path + dual-homed NIC + golden images + the `vm-lifecycle` worker; fold into DATACENTER/Instances.
