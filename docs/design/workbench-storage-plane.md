...
| 4 | UX | **GParted-authentic**: per-disk horizontal segment bar (colored, used/free shading) + partition table + the **pending-operations queue** — edits stage, nothing touches a disk until **Apply**, per-op progress streams during apply, staged ops reorder/undo. Rendered in Construct `Style`. |
