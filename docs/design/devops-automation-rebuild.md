...
- The drain/slot model is the EXISTING flock in `farm-dispatch.sh` + the `@farm-job` binding + inter-job snapshot-revert (`farm-vm-snapshot.sh reset`). No new scheduler. The flock is now per-SLOT (`<node>-slot<N>`), each slot owning its own remote workspace `~/magic-mesh-farm-d<N>`, so the fleet runs to the roster's declared ten slots instead of one job per node; admission is disk-shaped (heavy vs light headroom) because a cold whole-workspace `target/` outweighs any node's CPU cap. See `docs/BUILD-ENVIRONMENT.md` §4.
