...
> **Note (post-SUBSTRATE-6):** this wizard design says LH1 founds a **LizardFS
> master** + LH2/3 run **metalogger/QNM-Shared replicas** via `setup-qnm-shared`.
> **LizardFS is removed.** The substrate is now **etcd** (a quorum across the
> server+lighthouse anchors — replaces master/metalogger) + **Syncthing** (the
> file plane on a plain `/mnt/mesh-storage` dir — replaces the QNM-Shared mount/
> chunkservers), provisioned by `setup-etcd` / `setup-syncthing` instead of
> `setup-qnm-shared`. Read the LizardFS/QNM-Shared steps below as their etcd+
> Syncthing equivalents.
...
2. **Create a new mesh** (founds LH1): pick mesh-id → detect/confirm public IP →
   `mackesd found` (CA + self-sign + `/enroll` cert) → `setup-etcd --init` +
   `setup-syncthing` (was `setup-qnm-shared --master --chunkserver --client`) →
   `enable --now` every service → print the peer + lighthouse join lines → emit +
   run `site.yml`. Live log every step.
3. **Join an existing mesh**: enter a **lighthouse IP** + paste token (or scan) →
   ...
   (fp-pinned network enroll) → if Lighthouse: `setup-etcd --join` +
   `setup-syncthing` (was `setup-qnm-shared --chunkserver --client`) + register as
   nebula lighthouse in the roster → `enable --now` all → converge via Ansible.
...
5. **Status / services**: leader, node_count, per-service health (mackesd,
   nebula, etcd, syncthing, mesh-health — was qnm-shared/lizardfs), overlay
   reachability; restart any; re-run convergence.

## Service set the wizard guarantees running + boot-durable (ONBOARD-9 manager)
`nebula`, `mackesd`, `mesh-health.timer`, `etcd` (anchors) + `syncthing` (all) —
these replace the retired `qnm-shared`/`lizardfs-master`/`-chunkserver`/metalogger
set at SUBSTRATE-6 — all `enable --now` + auto-recovery + failure alerts. "Keep
them running" = the existing manager (Restart + watchdog).
...
- (Historical — retired by SUBSTRATE-6) LizardFS shadow-master auto-promotion
  (metalogger → master on LH1 loss) was manual in v1; with LizardFS removed,
  coordination HA is now etcd quorum (no master to promote).
