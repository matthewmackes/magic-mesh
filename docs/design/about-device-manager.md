...
hardware inspector** — a faithful categorized device tree, rendered in Construct dark,
...
| # | Area | Lock |
|---|------|------|
| 1 | Fidelity | **Faithful MDM layout, Construct-skinned** — the exact Device-Manager tree/columns/toolbar/interaction, rendered in `mde_egui::Style` dark tokens (not literal Win chrome, not a loose homage). |
...
- Opening **About** shows the device tree filling the body, brand as a title strip + an
  ⓘ dialog; the tree is the faithful MDM layout in Construct-dark, all-collapsed, with the
