...
| 4 | Chrome | **All of it** — a session/window/pane **sidebar tree** + a **native Construct status bar** + a **toolbar + command palette** + **context menus** on tabs/panes. |
...
| 8 | Status bar | **Native Construct status** (window tabs + session + clock), ignoring the user's tmux `status-*` config for a consistent look. |
...
- **Chrome** (`panel.rs`/new `tmux_ui.rs`): the sidebar tree, the native Construct status bar
...
- **Config**: a platform-managed Construct `.tmux.conf` (mesh-synced with the other browser/
...
- All via TERM's native tabs/splits/widget + the Construct `Style` tokens (§4); no second
...
## Out of scope (v1)
- Respecting arbitrary user `.tmux.conf` status themes (#8 = native Construct status).
