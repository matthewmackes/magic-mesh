...
| WL-UX-004 Brand spelling and product identity sweep | Completed | Current source, generated-user-facing metadata, install helpers, packaging, and non-archive docs now use canonical `Quazar` spelling; lower-case package/asset paths remain intentionally unchanged, and only two explicit historical S-spelling decision lines are allowed. | Commits `8a5935d9`, `2df459a1`, `5f610b26`, and `d842bc1f`; a follow-up Browser artifact identity pass corrected capture/PDF folders, MHTML/offline-copy subjects, and generated CUPS job titles to `Quazar Browser`; `install-helpers/lint-brand-identity.sh` scans the current docs tree plus source/install/package roots and falls back to `grep` on farm images without `rg`; local brand/worklist/diff checks and farm `.50` `brand-docs-lint` passed on 2026-07-17. | None. |
| WL-CRIT-003 Browser geometry and idle media regression | Completed | The reported Browser layout and idle-media regressions have code fixes, farm geometry/idle-media coverage, and installed live-seat proof on `.15`; the separate Browser visual-audit residual was later closed as WL-UX-002. | Commits `64508044` and follow-up Browser geometry/idle-media slices fixed the body bounds, horizontal-tab clipping, CEF media wake, PiP heartbeat, and idle-media pump paths. Farm proof on 2026-07-17 covered horizontal page-body bounds/right-edge coordinates on BigBoy `.130`, vertical body bounds on `.90`, many horizontal tabs on `.50`, focused `idle_media_pump`, CEF fmt, and Browser PiP heartbeat tests. BigBoy `.130` slot `browser-f44-current` built Fedora 44 split RPMs from commit `573dc609`; size guard passed (base 70.1 MiB, Browser 39.1 MiB). Artifacts staged to `.15` at `/home/mm/browser-f44-live-proof-573dc609/` with sha256 `8c03f2391bd33c42c6b7bcb5f405af1e9bd8cb4238d3845cb6b539ee2f649682` for `magic-mesh` and `9078bb47986fc366a8b8d85a68dd18f58cce06ffdbf54bbb16aff69b67811613` for `magic-mesh-browser`; `.15` passed `rpm -Uvh --test --replacepkgs --force --nosignature`, real replacement, clean `rpm -V`, explicit shell restart/start recovery, active new shell `MainPID=380954` at `2026-07-17 10:08:17 EDT`, running `/proc/380954/exe` hash matching `/usr/bin/mde-shell-egui`, installed CEF+Servo display/input verifier PASS, and installed CEF `--idle-media --budget 70 --timeout 90s` verifier PASS with process cleanup. | None. |
| WL-UX-002 Browser Chrome visual parity live audit | Completed | Browser chrome now has inspected representative visual snapshots, the final Browser Options layout regression found by those snapshots is fixed, and the item no longer needs to remain in the active worklist. | On 2026-07-17 the Browser Options page was changed to claim and paint the full available body rect and to constrain the wide category rail to `OPTIONS_RAIL_W`, fixing the visual defect where the command column was squeezed right and the body retained capture-clear background. The test-only screenshot harness now writes `browser-wide-vertical-options.png` and `browser-compact-horizontal-page.png`; BigBoy `.130` passed `cargo test -p mde-shell-egui browser_visual_audit_screenshots_cover_tab_modes_and_viewports -- --nocapture`, the regenerated screenshots were pulled to `/tmp/browser-visual-shot/` and visually inspected, BigBoy `.130` passed `browser_options_page_keeps_category_rail_when_wide`, farm `.50` passed `cargo fmt -p mde-shell-egui -- --check`, and `git diff --check` was clean. The installed `.15` Browser runtime proof remains the WL-CRIT-003 evidence; no new `.15` screenshot is claimed because passwordless sudo still returned unavailable from this shell. | None. |
| WL-RUN-007 Bus persistence index/file divergence repair | Completed | The Bus verifier now distinguishes real retained-message divergence from shared-root application JSON, exposes a repair path, and the live `.15` seat was repaired without silently dropping unread/operator data. The `seat-remote-input` worker no longer floods retained status files while idle and reopens a recreated Bus index before publishing. | Repository evidence: `crates/platform/mde-bus/src/persist.rs` adds typed divergence reporting, ignored non-message JSON, invalid message detection, idempotent reindex repair, explicit stale-row pruning with export, and repair tests; `crates/platform/mde-bus/src/cli/persist.rs` adds `mde-bus persist repair`; `crates/mesh/mackesd/src/workers/seat_remote_input.rs` reopens changed indexes and suppresses unchanged idle publishes. Farm evidence on 2026-07-18: `.50` passed `cargo fmt -p mde-bus --check` and `cargo fmt -p mackesd --check`; BigBoy `.130` passed `cargo test -p mde-bus repair_ -- --nocapture` and `cargo test -p mackesd workers::seat_remote_input --features async-services -- --nocapture`; `.90` built release `mde-bus` and `mackesd`. Live `.15` evidence: installed updated `/usr/bin/mde-bus` and `/usr/bin/mackesd`, ran `mde-bus persist repair --bus-root /run/mde-bus --prune-missing-rows --json`, exported the stale missing-row record under `.repair/`, and `mde-bus persist verify --bus-root /run/mde-bus --json` stayed clean over the final watch with zero new seat status files and active `mackesd` plus Sunshine services. | None. |
