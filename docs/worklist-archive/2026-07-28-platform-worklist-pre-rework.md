# Platform Worklist Pre-Rework Snapshot - 2026-07-28

> **NOT AN ACTIVE TRACKER.** The authoritative active worklist remains
> [`../platform/WORKLIST.md`](../platform/WORKLIST.md). This file preserves the
> complete pre-rework worklist, including completed-slice progress evidence and
> the detailed `WL-ARCH-008` Browser plan, from source revision
> `f52bad47fc1a35c408c4a5de2f2e004d32c49c5e`.

## Rework Dispositions

- **WL-ARCH-007 - Done.** Typed Workloads request contracts, explicit
  placement, replay-resistant mutation authority, scoped lifecycle behavior,
  hostile fixtures, farm gates, and the read-only installed-host verifier are
  implemented. The preserved ledger below records that direct KVM execution was
  unavailable on seat 15 because that host exposes neither `/dev/kvm` nor CPU
  virtualization flags; that optional hardware proof does not keep completed
  implementation active.
- **WL-FUNC-012 - Done.** The zero-cost overlay catalog, workers, retained
  mirrors, consumer bounds, staleness handling, projection, attribution, and
  farm tests are implemented. The preserved 2026-07-28 seat probe found an old
  `12.1.1-1` package missing current topics; repeating deployment proof with a
  newer RPM remains optional evidence rather than implementation work.
- **WL-FUNC-014 - Done.** The AirSonic gateway source model, proxy responder,
  sealed credential materialization, client session, cache behavior, role
  gates, and farm tests are implemented. A real two-node LAN playback run
  remains optional live evidence.
- **WL-FUNC-015 - Done.** The Jellyfin gateway model, source projection, proxy,
  range/progress forwarding, sealed account binding, failover, cache behavior,
  playback gates, and farm tests are implemented. A real cross-node playback
  run remains optional live evidence.
- **WL-UX-007 - Done.** The Car interface, always-fresh instrument strip,
  navigation, soft motion limits, MG90 adapter boundaries, deterministic pixel
  verifier, farm tests, and live seat/MG90 evidence are implemented.
- **WL-UX-008 - Done.** Workloads now has lifecycle routes, dense sortable
  tables, shared placement, expanded rows, health/review facts, exact-echo
  mutation review, reachable provisioning controls, and focused farm coverage.
  Additional seat capture remains optional visual evidence.
- **WL-UX-010 - Retired / absorbed by WL-FUNC-011.** Its completed interface
  slices and all unfinished requirements move to the single Mesh Teams product
  epic. The ID remains reserved and must not be reused.

## Verbatim Pre-Rework Worklist
...
- Priority: P0
- Complexity: Epic
- Problem: Construct still carries a large native host Browser implementation
  built around CEF/Servo offscreen helpers, shared-memory frame copies, CPU
  conversion, and shell texture uploads. Video refresh is visibly glitchy, a
  busy Browser can compete with the shell repaint loop, and the host stack
  contradicts the current thin-client architecture lock that applications run
  inside VM guests. Browser code, workers, package assets, and mesh integrations
  are also spread across shell, daemon, packaging, and mixed-purpose files, so
  deleting only the obvious helper crates would leave dead runtime and package
  seams behind.
- Required outcome: The complete current host Browser stack exists in an
  independent, history-preserving, clean-clone-buildable repository named
  `matthewmackes/magic-mesh-browser-stack`, with no path or build dependency on a
  sibling `magic-mesh` checkout. `magic-mesh` contains no CEF/Servo host Browser
  engine, Browser helper, Browser worker family, Browser-specific RPM, or
  Browser runtime installer. Construct retains one `Browser` navigation
  destination, but that destination only starts or resumes a dedicated
  `browser-vm`, attaches through the existing VDI stack, renders the guest
  framebuffer, and forwards focused input. Chromium, its tabs and chrome,
  network activity, media decode, and page failures remain inside the guest;
  the host shell remains the DRM-seat owner and stays responsive if the Browser
  VM stalls, disconnects, or crashes.
- Scope: Execute this as one ordered cross-repository cutover. Do not delete
  source or package assets from `magic-mesh` until the extracted repository has
  been pushed, its provenance recorded, and its standalone gates have passed.
  Do not retain the old host stack as a production fallback after the cutover.

  Phase 0 - baseline and ownership manifest:

  1. Record the exact source commit and branch used for extraction, the current
     workspace members/excludes, Browser RPM payload, helper/service inventory,
     daemon worker registrations, shell routes, persistent Browser state
     locations, and focused test baselines. Use `git ls-files`, manifests, and
     `rg` evidence rather than a hand-maintained filename guess.
  2. Classify every discovered component as Browser-owned, mixed-purpose, or
     shared. Extract Browser-owned code. Split Browser-only sections out of
     mixed files. Keep shared crates such as Bus, sealing, bookmarks, ad-block,
     VDI, and Workloads in `magic-mesh` only where a non-Browser live caller
     remains.
  3. Save the inventory and source-to-destination path map in the new
     repository's `UPSTREAM-SOURCE.md`. Include the original remote, immutable
     source commit, extraction method, mixed-file follow-up commits, and the
     `magic-mesh` removal commit once it exists. Never run history rewriting in
     the live `magic-mesh` worktree.

  Phase 1 - create the standalone Browser repository:

  1. Create a disposable clone, use a history-preserving filter operation for
     Browser-owned paths, and publish the result as
     `matthewmackes/magic-mesh-browser-stack`. Match the source repository's
     visibility and licensing unless the operator explicitly changes them.
     Preserve author attribution, tags relevant to the extracted paths,
     `LICENSE`, and `NOTICE`.
  2. Extract the old shell Browser UI under a standalone Browser application
     crate, plus `mde-web-preview`, `mde-web-cef`, `mde-web-sandbox`,
     `mde-web-wire`, `mde-web-preview-client`, and `mde-browser-workers`.
     Preserve the CEF renderer bridge, Servo helper, wire protocol, tests,
     Browser MPRIS/media behavior, download lane, session/policy/passkey/offline
     workers, and all other Browser-owned behavior even though Construct will no
     longer ship it.
  3. Extract Browser packaging and operations material: the Browser RPM
     definition, CEF and Widevine provisioning, WebExtension smoke assets,
     voice/read-aloud/translation models and helpers, SELinux policies, systemd
     units, runtime verification scripts, and current Browser design/operations
     documentation.
  4. Split Browser-only code currently embedded in `mackesd`, transfer lanes,
     KDC media projection, shell data-center policy, shell MPRIS wiring, and
     package-generation manifests into equivalent files in the new repository.
     Retain an upstream path map so future archaeology can trace every extracted
     section.

  Phase 2 - make the extracted repository independently buildable:

  1. Add a top-level workspace, pinned toolchain, lockfiles, README, development
     commands, CI definition, package instructions, and a small local
     compatibility layer for the shell, Bus, worker, sealing, bookmark,
     ad-block, and mesh-type interfaces used by the old Browser. The
     compatibility layer must implement only contracts needed by the extracted
     stack; it must not import the rest of `magic-mesh` or point outside the
     repository.
  2. Keep CEF and Servo as nested or excluded workspaces where their native
     runtime and SQLite dependency constraints require separate lockfiles.
     Document and test the exact top-level, CEF, Servo, renderer, verifier, and
     package build commands.
  3. A clean clone in an empty directory must resolve every path dependency
     within the new repository, build the standalone Browser application,
     compile/test the Browser wire/client and worker crates, and build both
     helper engines through their manifest paths. Tests may use fixtures, but
     release artifacts may not rely on fake engines, stub workers, or files from
     the original checkout.
  4. Clearly label the project as the standalone legacy host-Browser stack and
     state that Construct production releases use the VM Browser. Do not add a
     submodule or Git dependency from `magic-mesh` back to the extracted repo.

  Phase 3 - preserve user state before host-stack removal:

  1. Inventory legacy profiles, bookmarks, history, downloads metadata,
     sessions, policies, passkeys, offline cache, extension state, and model
     assets. Package removal must never recursively delete user data or shared
     downloads.
  2. Provide a one-shot, idempotent migration/export path for portable state
     that can be imported into guest Chromium. Back up original files before
     mutation, report imported/skipped/failed record counts, and make retries
     safe.
  3. Do not silently export private passkey material, cookies, passwords, or
     sealed credentials into the guest. Preserve them in their original sealed
     form or an encrypted operator-owned archive, document what cannot be
     migrated safely, and require explicit user action for any credential
     transfer.
  4. Keep rollback data independent of the retired RPM. A prior release may be
     reinstalled for emergency recovery, but the new `magic-mesh` release must
     not contain a hidden CEF/Servo fallback path.

  Phase 4 - remove the host Browser stack from `magic-mesh`:

  1. Remove all `mde-web-*` and `mde-browser-workers` crates from the root
     workspace, excludes, lockfile, shell and daemon dependencies, feature
     graphs, tests, and build scripts. Remove `live-helper`,
     `BrowserEngine::{Cef, Servo}`, `MDE_WEB_*`, `MDE_CEF_*`, helper discovery,
     process spawning, shared-memory Browser frame handling, and host Browser
     performance logging.
  2. Remove Browser worker re-exports and supervisor registrations for
     passkeys, offline cache, read-aloud, voice, translate, share, protocol
     handoff, tab suspend, session sync, and Browser policy. Remove Browser-only
     transfer, MPRIS, KDC media, data-center policy, and action-topic consumers.
     Preserve shared transfer, KDC, media, policy, and crypto behavior only when
     another reachable feature still uses it.
  3. Retire the `magic-mesh-browser` RPM and its weak dependency from the base
     package. Remove helper binaries, CEF runtime setup, Widevine setup,
     WebExtension/model installers, Browser SELinux modules, Browser systemd
     units, verification wrappers, post-install hooks, and uninstall hooks from
     Construct packaging and payload tests.
  4. Remove or supersede active documentation that describes a host Browser.
     Reconcile the stale Browser-chrome exception and CEF farm notes in
     `AI_GOVERNANCE.md` with the already-governing VM-application lock. Move
     Browser implementation documentation into the standalone repository and
     leave only VM Browser operations and migration guidance in `magic-mesh`.
     Historical archive evidence remains historical and is not rewritten.
  5. Use negative reachability and package tests to prove no host helper can be
     built, installed, enabled, or spawned. Residual uses of the word `Browser`
     are allowed only for the VM-backed shell destination, migration guidance,
     generic browser content, worklist/history, and intentional shared concepts
     that still have non-Browser callers.

  Phase 5 - implement the VM-backed Browser surface:

  1. Keep `Surface::Browser`, Dock/Front Door routing, shell navigation, and
     honest toast/error routing. Replace the old `web` state with a small
     `browser_vm` controller that uses typed Workloads and VDI contracts instead
     of a page engine or helper process.
  2. Define a reusable `browser-vm` profile using the existing
     `DeliveryType::DesktopVm` model rather than adding a Browser-specific
     delivery enum. The default profile is 4 vCPU, 8 GiB RAM, and 64 GiB disk,
     operator-tunable through Workloads. Its golden image contains Chromium, a
     supported GPU/video acceleration path, PipeWire audio integration, the
     guest agent, and the supported RDP service.
  3. On Browser activation, resolve the stable Browser workload, request a
     typed start/resume when needed, wait for its advertised desktop source,
     and attach with RDP as the required primary path. Use SPICE and then VNC
     only as explicit degraded fallbacks. Never silently pass fallback video as
     the primary performance proof.
  4. Render the guest through the existing VDI texture path and preserve damage
     rectangles through decode, frame application, and
     `TextureHandle::set_partial`. Add the Browser-specific attachment,
     reconnect, frame cadence, damage/full-upload ratio, decode/apply time,
     texture-upload time, and shell CPU/repaint metrics needed to diagnose
     regressions.
  5. Forward pointer, keyboard, text, scroll, and supported clipboard events
     only while the VM viewport owns focus. Construct-reserved navigation,
     session switching, emergency controls, status chrome, and global shortcuts
     remain host-owned and usable while Browser is active. Clipboard convergence
     uses the VDI lane owned by WL-FUNC-016; do not recreate shell-mediated web
     clipboard code.
  6. Route guest playback audio into the host PipeWire graph. The current RDP
     connector has audio playback disabled, so enable and prove the supported
     RDP audio channel or add an equally typed guest-audio stream under this
     epic. A fallback without audio must be labeled as degraded rather than
     appearing successful.
  7. Switching away from Browser releases input focus and stops unnecessary
     host texture uploads/repaints; it does not implicitly destroy the VM.
     VM stop/suspend follows explicit Workloads policy. Returning reattaches to
     the same guest session. Disconnects use bounded retry/backoff and an honest
     unavailable screen with Retry, Open Workloads, and diagnostic actions.
  8. Construct v1 does not mirror guest tabs, omnibox, extensions, or browser
     chrome. Chromium inside the guest owns those surfaces. A guest crash,
     decoder stall, or network disconnect may degrade the Browser viewport but
     must not block, crash, or continuously repaint the host shell.

  Phase 6 - cutover and prove both repositories:

  1. Land and push the standalone repository first. Record its default-branch
     commit and clean-clone evidence in this epic before merging the
     `magic-mesh` deletion and VM cutover.
  2. Land the `magic-mesh` removal and VM Browser as a reviewable sequence that
     keeps the tree buildable at each commit. The production cutover commit must
     identify the standalone repository commit containing the final extracted
     source.
  3. Update release/RPM payload expectations, install/upgrade tests, migration
     notes, and rollback notes. An upgrade must remove obsolete services and
     packages without deleting user data, then present the VM Browser or an
     honest provisioning/unavailable state.
  4. If remote repository creation, source push, or clean-clone standalone
     verification is unavailable, stop before deletion and change this epic to
     `Blocked` with the exact external gate. Do not treat a local unpushed split
     as sufficient preservation.
- Relevant files/components: root `Cargo.toml` and `Cargo.lock`;
  `crates/desktop/mde-shell-egui/src/web/`,
  `crates/desktop/mde-shell-egui/src/main.rs`,
  `crates/desktop/mde-shell-egui/src/surfaces.rs`,
  `crates/desktop/mde-shell-egui/src/datacenter.rs`, and
  `crates/desktop/mde-shell-egui/Cargo.toml`; all
  `crates/desktop/mde-web-*` crates; `crates/mesh/mde-browser-workers/`;
  `crates/mesh/mackesd/Cargo.toml`,
  `crates/mesh/mackesd/src/bin/mackesd/spawn.rs`,
  `crates/mesh/mackesd/src/workers/mod.rs`,
  `crates/mesh/mackesd/src/workers/transfers/lane/browser_media.rs`, and
  `crates/mesh/mackesd/src/workers/kdc_host/media.rs`; `packaging/browser/`,
  Browser-related files under `packaging/systemd/` and `packaging/selinux/`,
  Browser/CEF/Widevine/model scripts under `install-helpers/`;
  `crates/desktop/mde-shell-egui/src/vdi/`, `crates/desktop/mde-vdi-core/`,
  `crates/desktop/mde-vdi-rdp/`, `crates/desktop/mde-vdi-spice/`, and
  `crates/desktop/mde-vdi-vnc/`; `mackes-mesh-types` Workloads contracts,
  `mackesd` cloud/session/console/desktop-source workers, bootc/osbuild image
  inputs, OpenTofu/Ansible workload definitions, RPM payload verification,
  `AI_GOVERNANCE.md`, `docs/BUILD-ENVIRONMENT.md`,
  `docs/design/browser-*.md`, `docs/design/quasar-vdi-desktop.md`, and
  `docs/design/rpm-size-split.md`; planned sibling checkout
  `/root/magic-mesh-browser-stack` and remote
  `matthewmackes/magic-mesh-browser-stack`.
- Dependencies: Reuse the placement, authorization, lifecycle, and console
  contracts repaired by WL-ARCH-007; extraction and standalone-repository work
  may proceed before that epic closes, but the production Browser attach must
  not bypass those contracts. Coordinate VDI clipboard with WL-FUNC-016 and
  shared shell appearance/unavailable states with WL-UX-009. Neither linked UX
  epic may reintroduce host Browser chrome or block source preservation.
- Acceptance criteria:

  1. The remote `matthewmackes/magic-mesh-browser-stack` exists, records the
     immutable source commit and path map, preserves relevant file history and
     authorship, contains all old Browser-owned source/assets/docs, and passes
     documented gates from a clean clone with no sibling checkout.
  2. The standalone repository builds/tests its top-level workspace and its
     separate CEF and Servo manifests, produces the old helper/verifier and
     Browser package artifacts, and contains no fake production engine or
     dependency on an uncommitted local compatibility shim.
  3. `magic-mesh` has no `mde-web-*` or `mde-browser-workers` workspace member,
     dependency, feature, helper binary, worker spawn, package payload, unit,
     SELinux policy, runtime installer, or shipped CEF/Servo/Widevine asset.
     `magic-mesh-browser` is absent from generated RPMs and dependency metadata.
  4. Existing Browser data is inventoried and backed up before package removal;
     portable data migration is idempotent and reports results; downloads are
     preserved; credentials and passkeys are never silently copied or exposed.
  5. Opening Browser starts or resumes the typed `browser-vm` workload and
     displays its Chromium desktop through live RDP. With no VM/source, the
     shell shows actionable unavailable state and remains fully navigable.
  6. While Browser has focus, page input reaches the guest and shell-reserved
     controls stay local. Switching surfaces stops unnecessary Browser texture
     uploads without killing the guest, and returning resumes the same session.
     A guest or transport crash never terminates or wedges Construct.
  7. Guest `vainfo` succeeds for the selected device and Chromium media
     diagnostics report a GPU video decoder for the acceptance stream. The host
     process list contains no `mde-web-cef`, `mde-web-preview`,
     `mde-web-cef-renderer`, or `cef-verify` while Browser is open.
  8. Five concurrent 1080p video tabs run for at least 15 minutes. The visible
     tab sustains at least 90 percent of source cadence up to the supported
     target, with a minimum supported target of 30 fps, no VDI frame stall over
     500 ms that is not attributable to source buffering, and continuous frame
     delivery for five minutes with the pointer stationary and no shell input.
  9. During that load, Construct navigation and session switching have p95
     response at or below 100 ms and no response over 250 ms in the measured
     run. Damage metrics show partial uploads on RDP updates, hidden Browser
     state does not drive continuous shell repaint, and host CPU/GPU measurements
     are recorded with the acceptance artifact.
  10. Browser audio plays through the host PipeWire graph, appears in the normal
      VM/application mixer path, follows mute/volume controls, and survives
      Browser surface switching according to policy. Silent fallback is
      explicitly degraded and cannot satisfy the release gate.
  11. A clean install and an upgrade from the last host-Browser release both
      pass. Obsolete units are disabled/removed, package verification is clean,
      no user data is deleted, and reinstalling a prior signed release remains
      a documented emergency rollback without restoring an in-tree fallback.
  12. Governance and current operations docs describe only the VM Browser for
      Construct. The standalone repo owns host Browser implementation docs;
      historical archived evidence remains intact and clearly historical.
- Verification method: Run `install-helpers/lint-worklist.sh --self-test`,
  `install-helpers/lint-worklist.sh`, repository boundary/supersession/secret
  lints, and `git diff --check`. On the farm, put clean-clone standalone
  workspace plus CEF/Servo builds and the `magic-mesh` workspace/RPM cut on
  BigBoy; spread focused shell VDI, RDP, Workloads, `mackesd`, migration, and
  payload tests across the remaining verified farm hosts. Required current-repo
  gates include
  ...
  `@farm:{cargo test --workspace}`, and
  `@farm:{cargo clippy --workspace --all-targets --all-features -- -D warnings}`.
  In the new repository, run its documented top-level workspace tests and
  manifest-path CEF/Servo build/test commands from a fresh clone. Complete live
  acceptance on a GPU-capable Workstation with timestamped VDI metrics,
  stationary-pointer video capture, Chromium media diagnostics, PipeWire audio
  proof, process/RPM inventory, disconnect/reconnect drill, and simultaneous
  shell-navigation latency capture. Use Sunshine/Moonlight or equivalent
  rendered-output capture when available; otherwise record hardware
  unavailability explicitly without claiming the live criteria passed.
- Origin or merged source IDs: 2026-07-28 operator Option 3 decision: remove the
  current Browser stack from `magic-mesh`, move everything Browser into a new
  buildable standalone repository, and make the production Browser a VM-backed
  VDI surface. Corrective architectural successor to archived WL-PERF-003,
  WL-FUNC-001, WL-FUNC-002, WL-FUNC-003, WL-FUNC-004, and WL-ARCH-005; those
  completed historical epics stay archived and are not reopened. Primary
  evidence source: `docs/design/browser-perf-native.md`.
