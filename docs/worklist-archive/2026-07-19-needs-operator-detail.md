...
- **OW-11 service-add** — _needs:_ **DO Spaces creds** (Music → Navidrome, overlaps
  ...
  register.
- **OW-12** (parked 2026-07-03) — Construct/headless WS kickstart authored (packaging/kickstart/magic-on-quasar.ks, bash -n + shellcheck clean); remaining acceptance is LIVE-BOOT-GATED (boot the ISO to confirm display + headless WS onboard) + the .iso cut is OPERATOR-GATED (/release, incl. RPM signing + bootc registry publish)
...
- **MOTION-TRANS-4 + MOTION-PERF-4 → [✗] WON'T-DO** — their acceptance targets the retired iced/Cosmic compositor; carrying the polish to the egui/Construct shell would be net-new work, not completion (mirrors GUI-9).
...
- **AUTHORIZED (2026-07-03): standing prod-SSH + XCP cloud create/delete + maintenance window** (DAR DevOps-rebuild live tail) + **stand up the live Construct VDI test bed** (E12 VDI live legs).
...
- **NAMING-1: superseded legacy product naming — ✅ RESOLVED 2026-07-18; sweep tracked by `WL-UX-004`** _(review `docs-consistency-2` / `shell-ux-9`)._
  - **Decision.** The operator supplied the new Construct artwork and selected **"Construct"** as the visible product name and 12.x codename. **`magic-mesh` stays the package/repo/infra id** (the GNOME-vs-gnome-shell split, QBRAND #10), and compatibility file paths / internal identifiers are not renamed unless separately justified.
  - **Current state.** The brand crate implements the codename decision (`crates/shared/mde-theme/src/brand/build.rs` `12 => "Construct"`), and `brand::logo::PRODUCT_NAME = "Construct"`. The active sweep corrected Browser dashboard, phone endpoint/notification identity, Console provenance, Device Manager report/menu labels, Terminal preset/config labels, Editor About, package metadata, and the Construct raster/icon slots.
