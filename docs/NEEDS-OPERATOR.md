...
- **OW-11 service-add** — _needs:_ **DO Spaces creds** (Music → Navidrome, overlaps
  ...
  register.
- **OW-12** (parked 2026-07-03) — Quazar/headless WS kickstart authored (packaging/kickstart/magic-on-quasar.ks, bash -n + shellcheck clean); remaining acceptance is LIVE-BOOT-GATED (boot the ISO to confirm display + headless WS onboard) + the .iso cut is OPERATOR-GATED (/release, incl. RPM signing + bootc registry publish)
...
- **MOTION-TRANS-4 + MOTION-PERF-4 → [✗] WON'T-DO** — their acceptance targets the retired iced/Cosmic compositor; carrying the polish to the egui/Quazar shell would be net-new work, not completion (mirrors GUI-9).
...
- **AUTHORIZED (2026-07-03): standing prod-SSH + XCP cloud create/delete + maintenance window** (DAR DevOps-rebuild live tail) + **stand up the live Quazar VDI test bed** (E12 VDI live legs).
