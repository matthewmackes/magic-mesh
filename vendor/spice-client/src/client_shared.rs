...
impl SpiceClientShared {
    ...
    pub async fn start_event_loop(&self) -> Result<()> {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            // Inputs is a write-only channel. Its receive loop would hold the
            // channel mutex while waiting for server bytes, starving outbound
            // keyboard and pointer writes that need the same mutex.
