...
use crate::{SessionResult, custom_err, reason_err};
...
impl DecodedImage {
    ...
    fn require_bitmap_data_size(
        ...
        if source_width < update_rectangle.width() {
            return Err(reason_err!(
                "ApplyBitmap",
                "source width {} is narrower than destination width {}",
                source_width,
        ...
        let pixel_count = usize::from(source_width)
            ...
            .ok_or_else(|| reason_err!("ApplyBitmap", "bitmap rectangle dimensions overflow"))?;
        let expected_length = pixel_count
            .checked_mul(bytes_per_pixel)
            .ok_or_else(|| reason_err!("ApplyBitmap", "bitmap source dimensions overflow"))?;

        if data.len() != expected_length {
            return Err(reason_err!(
                "ApplyBitmap",
                "bitmap source has {} bytes; expected {} for {}x{} at {} bytes per pixel",
                data.len(),
                expected_length,
                source_width,
...
mod tests {
    ...
    #[test]
    fn clipped_bottom_row_preserves_source_stride_without_writing_past_framebuffer() {
        ...
        // One four-pixel source row is clipped to a two-pixel destination at
        // the lower-right edge. Chunking by the destination width would invent
        // a second row and index beyond the framebuffer.
        let bgr = [3, 2, 1, 6, 5, 4, 9, 8, 7, 12, 11, 10];

        image.apply_bgr24_bitmap(&bgr, &rectangle, 4).unwrap();

        assert_eq!(pixel(&image, 2, 1), [1, 2, 3, 255]);
        assert_eq!(pixel(&image, 3, 1), [4, 5, 6, 255]);
        assert_eq!(pixel(&image, 1, 1), [0, 0, 0, 0]);
    }

    #[test]
    fn rgb24_clipped_bottom_row_does_not_invent_an_extra_destination_row() {
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 4, 2);
        let rectangle = InclusiveRectangle {
            left: 2,
            top: 1,
    ...
    #[test]
    fn bitmap_application_rejects_inconsistent_source_geometry() {
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 4, 2);
        let rectangle = InclusiveRectangle {
            left: 0,
            top: 0,
        ...
        assert!(image.apply_bgr24_bitmap(&[0; 11], &rectangle, 4).is_err());
        assert!(image.apply_bgr24_bitmap(&[0; 13], &rectangle, 4).is_err());
        assert!(image.apply_bgr24_bitmap(&[0; 3], &rectangle, 1).is_err());
    }

    #[test]
    fn bitmap_destination_rejects_invalid_or_mismatched_rectangles() {
        let image = DecodedImage::new(PixelFormat::RgbA32, 4, 2);
        ...
        assert_eq!(image.bitmap_destination(&rectangle, 1, 1), None);
        assert_eq!(image.bitmap_destination(&rectangle, 2, 2), None);
        assert_eq!(image.bitmap_destination(&rectangle, 2, 1), Some(rectangle));
    }
}
