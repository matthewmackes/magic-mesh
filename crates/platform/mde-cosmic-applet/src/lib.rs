...
//! Applications-Panel launcher model (APPS-2) and the lighthouse-health panel
//! indicator (LIGHTHOUSE-7). The libcosmic shells that draw this state into the
//! Cosmic panel are the hardware-gated render targets (they need a Cosmic
//! session to build + verify); they consume this crate so their surface is thin
//! glue, not logic.
