...
//!   fired through the `mde-bus` CLI + [`crate::proc_reap::fire_and_reap`], and
//!   every virsh shell-out is bounded by the EFF-20 timeout so a wedged libvirt
...
/// Publish an [`InstanceReport`] to [`INSTANCES_TOPIC`] via the `mde-bus` CLI —
/// the same fire-and-reap path `kvm_health` uses. Best-effort: a missing
/// `mde-bus` binary (pre-RPM dev box) is swallowed, and the detached reaper
/// prevents a zombie pile.
fn publish_instances(report: &InstanceReport) {
    let Ok(body) = serde_json::to_string(report) else {
        return;
    };
    let mut cmd = Command::new("mde-bus");
    cmd.args(["publish", INSTANCES_TOPIC, "--body-flag", &body]);
