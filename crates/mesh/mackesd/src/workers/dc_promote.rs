...
/// One poll pass: resolve the build version + feed the three stages through the
/// dedup core, publishing the records that survive. The `build` stage carries
/// the resolved version (or `"unknown"`) as `ready`; the `eagle` + `do` stages
/// are best-effort `pending` placeholders at the honest `"unknown"` version
/// until those hosts are reachable.
fn poll_and_publish(core: &mut DcPromote, home: Option<&Path>, repo: Option<&Path>) {
    let build_version =
    ...
    for stage in ["eagle", "do"] {
        if let Some(rec) = core.observe(stage, UNKNOWN_VERSION, "pending") {
            publish(&rec);
        }
    }
}
...
impl Worker for DcPromoteWorker {
    ...
    async fn run(&mut self, mut shutdown: ShutdownToken) -> anyhow::Result<()> {
        ...
        loop {
            if self.is_leader() {
                poll_and_publish(&mut self.core, home.as_deref(), Some(repo.as_path()));
            }
            tokio::select! {
