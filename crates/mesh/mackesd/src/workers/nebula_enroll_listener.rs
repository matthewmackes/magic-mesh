...
/// persisted alongside the Nebula files. The TCP/443 relay reuses this
/// fingerprint-pinned identity by default so enrollment establishes both trust
/// relationships without a second bootstrap channel.
...
mod tests {
    ...
    async fn full_stack_pinned_client_enrolls_and_materializes() {
        ...
        let requester_key = crate::nebula_enroll_client::RequesterNebulaKey::for_test(&config_dir);
        let bundle = crate::nebula_enroll_client::network_enroll_with_requester_key(
            &peer_root,
            &config_dir,
            "peer:anvil",
            "anvil",
    ...
    async fn full_stack_fp_mismatch_is_refused_fail_closed() {
        ...
        let config_dir = tmp.path().join("etc-nebula");
        let requester_key = crate::nebula_enroll_client::RequesterNebulaKey::for_test(&config_dir);
        let result = crate::nebula_enroll_client::network_enroll_with_requester_key(
            &tmp.path().join("peer"),
            &config_dir,
            "peer:anvil",
            "anvil",
