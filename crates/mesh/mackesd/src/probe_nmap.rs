...
/// Resolve the full scan scope (Q5): enrolled mesh peers (from
/// `workgroup_root`) ∪ detected bounded LAN CIDRs ∪ observed individual
/// neighbours on wider LANs ∪ operator-arbitrary targets (from
...
pub fn resolve_targets(workgroup_root: &Path, home: &Path) -> TargetSet {
    // Full mesh peers, including explicitly enrolled guests, are resolved only
    // from their live peer identity bundles. Retired compute-inventory files
    // are never accepted as a parallel VM roster or network-identity source.
...
<hosts up="1" down="1" total="2"/>
    ...
    #[test]
    fn resolve_targets_ignores_retired_compute_inventory() {
        let root = tmp_root("resolve-retired-compute");
        let qnm = root.join("qnm");
        let retired = qnm.join("old-host");
        std::fs::create_dir_all(&retired).unwrap();
        std::fs::write(
            retired.join("compute-inventory.json"),
            r#"{"vms":[{"name":"stale","nebula_ip":"203.0.113.77"}]}"#,
        )
        .unwrap();

        let targets = resolve_targets(&qnm, &root.join("home"));
        let _ = std::fs::remove_dir_all(&root);
        assert!(
            !targets
                .targets
                .iter()
                .any(|target| target == "203.0.113.77"),
            "retired runtime inventory cannot inject a network scan target"
        );
    }
}
