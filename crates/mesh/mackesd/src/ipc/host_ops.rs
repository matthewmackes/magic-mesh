...
pub const ACTION_VERBS: [&str; 10] = [
    ...
    "gateway-firewall",
    "gateway-portforward",
    // DATACENTER-17 — day-2 host care: evacuate-first patch + evacuate.
    "host-evacuate",
    "host-patch",
];
...
pub fn build_reply(svc: &HostOpsService, verb: &str, req_body: Option<&str>) -> String {
    ...
    match verb {
        ...
        // DATACENTER-17 — day-2 host care: evacuate, and the evacuate-first patch.
        "host-evacuate" => {
            return match host_day2(req_body, "host-evacuate", host_evacuate_remote()) {
                Ok(()) => json!({ "ok": true }).to_string(),
                Err(m) => err(m),
            };
        }
        "host-patch" => {
            return match host_day2(req_body, "host-patch", host_patch_remote()) {
                Ok(()) => json!({ "ok": true }).to_string(),
                Err(m) => err(m),
            };
        }
