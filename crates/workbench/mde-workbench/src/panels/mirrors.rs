...
fn mirror_row<'a>(r: &'a MirrorRow, palette: Palette) -> Element<'a, crate::Message> {
    ...
    let (sync_color, sync_text) = match r.last_sync_ms {
        Some(ms) => (palette.success.into_iced_color(), fmt_age_ms(ms)),
        None => (
    ...
    let (state_color, state_text) = if r.enabled {
        (palette.success.into_iced_color(), "enabled")
    } else {
        (palette.text_muted.into_iced_color(), "disabled")
    };
    ...
    let head = row![
        ...
        text(r.name.clone())
            .size(12)
            .color(palette.text.into_iced_color()),
        text(state_text).size(10).color(state_color),
        Space::new().width(Length::Fill),
        text(sync_text).size(10).color(sync_color),
    ]
    .spacing(8)
    .align_y(iced::alignment::Vertical::Center);
