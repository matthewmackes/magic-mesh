...
/// A room's **signed, replicated membership descriptor** (lock 7/25): the id,
/// the display name, and the member hostnames.
///
...
impl Room {
    /// A room `id`/`name` with the [`DEFAULT_CAPACITY`] shared log and no members
    /// yet. The `creator` is joined immediately.
    #[must_use]
