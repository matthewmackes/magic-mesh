...
pub fn parse_dial_descriptor(body: &[u8], address: &str) -> io::Result<CastDiscovery> {
    ...
    let name = tag(text, "friendlyName")
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Cast descriptor has no name"))?;
    let udn = tag(text, "UDN")
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "Cast descriptor has no UDN"))?;
    let model = tag(text, "modelName").unwrap_or_else(|| "unknown".to_owned());
    Ok(CastDiscovery {
        target: CastTarget::new(address, name)?,
        udn,
        model,
    })
}
...
pub fn commit_cast_ownership(path: &Path, target: &CastTarget, generation: u64) -> io::Result<()> {
    let record = CastOwnership {
        target_id: format!("cast:{}", target.address),
        generation,
    };
    ...
    pub fn load(url: &str, content_type: &str, start_seconds: f64) -> io::Result<Self> {
            ...
            || url.chars().any(char::is_control)
        {
            return Err(io::Error::new(
                ...
                "Cast media URL must be a bounded HTTP(S) URL",
            ));
        }
            ...
            || content_type.chars().any(char::is_control)
        {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "invalid Cast media content type",
            ));
        }
        if !start_seconds.is_finite() || start_seconds < 0.0 {
            return Err(io::Error::new(
                io::ErrorKind::InvalidInput,
                "invalid Cast start position",
            ));
        }
        Ok(Self::Load {
            url: url.to_owned(),
...
pub fn execute_cast_command(target: &CastTarget, command: &CastCommand) -> io::Result<()> {
    use rust_cast::channels::media::{LoadOptions, Media, StreamType};
...
mod tests {
    ...
    fn target_requires_numeric_address_and_bounded_name() {
        ...
        assert_eq!(
            CastTarget::new("cast.local", "TV").unwrap_err().kind(),
            io::ErrorKind::InvalidInput
        );
        assert_eq!(
            CastTarget::new("172.20.146.150", "\n").unwrap_err().kind(),
            io::ErrorKind::InvalidInput
        );
    }
    ...
    fn ownership_commit_is_generation_bound_and_atomic() {
        ...
        let record: super::CastOwnership =
    ...
    fn cast_handoff_reuses_bounded_media_contract() {
        let handoff =
            super::CastHandoff::new("https://music.example/song.mp3", "audio/mpeg", 12.5, 7)
                .unwrap();
    ...
    fn live_castv2_connection_when_operator_supplies_target() {
        ...
        verify_castv2(&target)
            .expect("operator-supplied Cast target must accept CASTV2 connection");
    }
    ...
    fn live_cast_media_load_and_pause_when_operator_supplies_url() {
        ...
        execute_cast_command(&target, &CastCommand::Pause)
            .expect("Cast receiver must accept pause");
    }
