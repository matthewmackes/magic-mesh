...
//! [`Palette`] of the same shape the Construct default ([`Palette::from_tokens`])
//! does, so the renderer treats a preset and the default identically.
//!
//! [`Preset`] is the pickable set (the Construct default plus the four classics)
...
/// A user-pickable colour scheme: the Construct default (token-derived) plus the
...
impl Preset {
    ...
    pub const fn palette(self) -> Palette {
        match self {
            Self::Construct => Palette::from_tokens(),
...
mod tests {
    ...
    #[test]
    fn platform_default_label_uses_canonical_construct_identity() {
        assert_eq!(Preset::Construct.label(), "Construct");
        assert!(
            !Preset::Construct.label().contains(concat!("Qua", "sar")),
    ...
    fn matching_round_trips_each_preset_and_default() {
        ...
        // The token default resolves to the Construct preset (its palette equals it).
        assert_eq!(
            Preset::matching(&Palette::from_tokens()),
            Some(Preset::Construct)
        );
