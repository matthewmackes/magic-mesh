...
//! **Quazar default, derived from `mde-theme`/`Style` tokens wherever a token
...
//! are standard ANSI hues tuned for the dark Quazar background.
//!
...
// ── The 16 ANSI slots of the Quazar default (0..=7 normal, 8..=15 bright) ────
...
/// The Quazar default 16-slot ANSI table, indexed by slot number.
...
impl Palette {
    /// The **Quazar default**, derived from `mde-theme`/`Style` tokens — the
...
/// Resolve a 256-colour palette slot against the **Quazar default**.
///
...
///   text *is* the active scheme (the Quazar default's are the platform tokens);
...
mod tests {
    ...
    fn the_16_slot_table_is_wired_and_token_derived_where_it_can_be() {
        ...
        // The Quazar-token derivations (the §4 carve-out's "derive where
        // sensible" clause).
    ...
    fn default_cells_are_the_palette_role_colours() {
        ...
        // The Quazar default's role colours are the chrome tokens.
