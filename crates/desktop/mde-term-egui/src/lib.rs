...
//!   carve-out): Construct-token-derived where a token carries the meaning,
...
//!   Construct default derived from `Style` tokens, and [`presets`] holds the
...
//!   adds the native chrome: the Construct status bar (session · window list ·
