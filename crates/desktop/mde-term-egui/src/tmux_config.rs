//! TMUX-FC-8 — the **platform-managed Quazar tmux config** (mesh-synced) + its
//! application over the control channel.
//!
//! Design: `docs/design/tmux-first-class.md` (#14 config: a Quazar default,
...
//! server always reflects the Quazar config regardless of what `.tmux.conf` (if
...
/// The default scrollback history limit (lines) — the Quazar default, matching a
/// generous-but-bounded terminal.
const DEFAULT_HISTORY: u32 = 50_000;

/// The platform-managed Quazar tmux config: the operator-tunable knobs.
...
impl TmuxConfig {
    ...
    /// The equivalent `.tmux.conf` text — the canonical Quazar config artefact
    ...
    pub fn to_conf(&self) -> String {
        let prefix = self.safe_prefix();
        format!(
            "# Quazar tmux config (platform-managed, mesh-synced) — TMUX-FC-8\n\
...
impl TmuxConfigStore {
    ...
    /// Load the mesh config — the Quazar [`TmuxConfig::default`] when absent,
...
mod tests {
    ...
    #[test]
    fn to_conf_renders_the_equivalent_quazar_file() {
        let conf = TmuxConfig::default().to_conf();
        assert!(conf.starts_with("# Quazar tmux config"));
        assert!(
