...
/// Render the Android VM view — the two-layer Cuttlefish roster + outer-VM
/// lifecycle + inner-cvd console.
pub(super) fn view(ui: &mut egui::Ui, state: &mut WorkloadsState) {
    heading(
        ui,
        "Android VM",
        "Android via two-layer Cuttlefish: an L1 Linux VM hosts cvd, which boots Android inside.",
    );
    provision_cta(ui, state, "Provision an Android VM");
...
/// drift, the outer-domain metrics, then the outer-VM verbs (+ inner-cvd console).
fn android_card(ui: &mut egui::Ui, state: &mut WorkloadsState, row: &WorkloadRow) {
    card().show(ui, |ui| {
        header_row(ui, row);
        metrics_line(ui, row);
...
/// The card's identity row: name (strong), the two-layer tag, the outer-domain
/// status dot + word, the drift chip, then the node.
...
/// A small recessed two-layer tag — the `inset` well around a dim caption, so the
/// L1-VM-hosts-cvd nature reads at a glance.
...
/// The live cpu / mem / disk metrics row for the outer L1 domain (cpu toned by
/// load).
...
fn heading(ui: &mut egui::Ui, title: &str, blurb: &str) {
    ui.horizontal(|ui| {
        ui.scope(|ui| {
            ...
            carbon_icon(ui, DeliveryView::AndroidVm.icon(), Style::BODY + 2.0);
        });
        ui.add_space(Style::SP_XS);
        ui.label(
            RichText::new(title)
