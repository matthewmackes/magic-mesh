---
name: drain-worklist
description: Drain docs/platform/WORKLIST.md by fanning disjoint parallel workers across the MCNF Xen build farm under the AI_GOVERNANCE.md §10.0.4 Parallel drain contract. Use when the operator asks to drain the worklist, drain worklist fully, fan out, utilize the farm, use full farm capacity, keep agents building, run parallel drain, why we are not working on more epics, or reports the farm is idle. Each worker gets a disjoint write scope and runs its @farm:{cargo …} unit on an admitted farm slot; the parent folds completed workers, commits + pushes under standing auth, and re-fans against surfaced blockers on every tick. Supersedes older drain skills that predate the 2026-08-20 §10.0.4 lock and any stale 4-node / 9-slot topology.
---
...
6. **Keep working locally** on non-conflicting critical-path work while
   sidecars run. Do not block on their result unless the next step
   requires it.
7. **Fold completions** as they return. Commit (with standing auth)
   ...
   never revert to obtain a clean receipt. Then
   `automation/reconciler/tick-fill.sh` — do not wait for the timer.
...
Source of truth: `install-helpers/farm-topology.sh`. Current shape
(2026-08-20 — supersedes any older 4-node / 9-slot text in home-level
skills):
...
- Design polish (Codex has world-class-gui-polish; MCNF has polish).
