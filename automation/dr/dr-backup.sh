...
# for the secrets (age-in-age) and single-age for the tofu state.
#
# CAVEAT (printed at the end): the mesh age IDENTITY (private key) and the
# Nebula CA can NOT be recovered from this file — the master key cannot live
# only inside the thing it decrypts. Back those up SEPARATELY and securely.
#
# Env:
#   MCNF_ETCD     etcd v3 gateway (http://172.20.145.192:2379)
#   MCNF_AGE_KEY  mesh age identity, used only to derive the recipient (/root/.mcnf-age-key)
...
# Assemble the manifest from the three sources. Python merges the kvs arrays,
# keeping only key+value (base64, as etcd returns them) so a restore can re-put
# them verbatim.
...
MANIFEST="$(
  TS="$TS" RECIP="$RECIP" python3 - "$TOFU_JSON" "$SECRET_JSON" "$RECIP_JSON" <<'PY'
import sys, json, os
