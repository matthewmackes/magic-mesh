...
# re-puts every key into etcd, REWRITING the key prefix. By default it restores
# under a TEMP prefix (/dr-restore-test/) so a round-trip can be verified
# without ever touching the live /tofu/ or /mcnf/ trees. Pass --prod to restore
# to the ORIGINAL keys (clobbers production) — deliberately opt-in.
#
# Prefix rewrite: each original key is mapped by replacing its leading "/" with
# the target prefix. e.g. with target "/dr-restore-test/":
...
if [ "$PROD" -eq 1 ]; then
  echo "restored $COUNT keys to PRODUCTION (original keys) from $FILE"
else
  echo "restored $COUNT keys under prefix '$PREFIX' from $FILE"
fi
