...
#   mcnf-secret.sh put <name>           encrypt stdin → etcd /mcnf/secret/<name> (to the FULL recipient set)
#   mcnf-secret.sh get <name>           decrypt etcd /mcnf/secret/<name> → stdout (exit 3 if absent)
#   mcnf-secret.sh rotate <name> [--revoke-cmd '<cmd>']
#                                       atomic overwrite of <name> + optional provider-side revoke
#   mcnf-secret.sh reseal-to <recipient>  re-encrypt every secret to {registered set} ∪ {<recipient>}
#   mcnf-secret.sh reseal-all           re-encrypt every secret to the full registered recipient set
#   mcnf-secret.sh recipients           list the recipient SET (public keys only) used for sealing
...
case "$cmd" in
  ...
  put)
    ...
    # DAR-3: seal to the FULL recipient set (every registered identity can read).
    ...
    _put "/mcnf/secret/$2" "$ct"; echo "stored /mcnf/secret/$2 (recipients: ${#RSET[@]})"
    ;;
  ...
  rotate)
    _resolve_etcd
    [ -n "${2:-}" ] || { echo "usage: rotate <name> [--revoke-cmd '<cmd>']" >&2; exit 2; }
    name="$2"; shift 2
    revoke_cmd=""
    while [ $# -gt 0 ]; do
      case "$1" in
    ...
    # Read the NEW value from stdin, seal to the full set, atomic single-key put.
    mapfile -t RSET < <(recipient_set)
    [ "${#RSET[@]}" -gt 0 ] || { R="$(recipient)"; RSET=("$R"); }
    ct="$(_seal_to_set "${RSET[@]}")"
    _put "/mcnf/secret/$name" "$ct"
    echo "rotated /mcnf/secret/$name (recipients: ${#RSET[@]})"
  ...
  reseal-to|reseal-all)
    ...
    # Build the target recipient set: registered union ∪ {extra}.
    mapfile -t RSET < <(recipient_set)
    [ -n "$extra" ] && RSET+=("$extra")
    ...
    for full in "${NAMES[@]}"; do
      ...
      # Decrypt with the local (current mesh) key, re-seal to the new set, atomic put.
      # NEVER echo the plaintext: the pipe goes age-d → age-seal → b64 in-process.
      if ! ct="$(age -d -i "$KEY" <"$tmp" | _seal_to_set "${RSET[@]}")"; then
        rm -f "$tmp"
  ...
  recipients)
    ...
    # the ONLY secret-store data this tool ever emits.)
    recipient_set
    ;;
  ...
  *)
    echo "usage: $0 {init|init-self|put <name>|get <name>|rotate <name>|reseal-to <recipient>|reseal-all|recipients|list|selftest}" >&2
    exit 2
    ;;
esac
