#!/usr/bin/env python3
"""Hostile producer and revalidation tests for Browser base-image receipts."""
...
def main() -> None:
    with tempfile.TemporaryDirectory() as temp:
        ...
        base = ("--repo",str(repo),"--skopeo",str(fake)); common=("--image-reference","registry.invalid/fedora/browser-base:44","--architecture","amd64","--source-revision",revision,"--commit-epoch","1700000000")
        ...
        value=json.loads(receipt.read_text()); assert value["browser_target"]=="mcnf-browser-vm/browser-vm-chromium-v1"; assert value["browser_profile"]=="browser-vm-chromium"; assert value["platform_digest"]==amd; assert stat.S_IMODE(receipt.stat().st_mode)==0o400
    ...
    print("Browser base-image receipt hostile self-test: PASS")
